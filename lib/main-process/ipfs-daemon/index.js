...
export default class IPFSDaemonController {
  ...
  startChild() {
    ...
    var startP = this.promises.startChild = this.ensureInitialized()
      ...
      .then(wrapper => {
        this.wrapper = wrapper // FIXME: what the fuck is happening to this promise later in the process, such that I can't resolve it and have to resort to setting an instance property like this!???
  ...
  kill() {
    if (this.wrapper && !this.wrapper.exited) {
      this.wrapper.process.kill('SIGKILL')
    }
    return this.waitProcess().catch(() => { }) // FIXME: catch errors so when it dies due to SIGKILL we still end up resolving the returned promise
  }

  shutdown() {
    this.shuttingDown = true
    return this.kill().then(() => { this.shuttingDown = false })
  }
