...
export default class PubTree {
  ...
  publish(ipfsClient) {
    return this.addToIPFS(ipfsClient).then(key => {
      ipfsClient.namePublish(key)
      return key
    })
  }
  ...
  updatePeers(newPeersList) {
    if (newPeersList.toJS instanceof Function) {
      this.peers = newPeersList.toJS()
    } else {
      this.peers = newPeersList
    }
  }

  updateTracks(newTracksList) {
    if (newTracksList.toJS instanceof Function) {
      this.tracks = newTracksList.toJS()
    } else {
