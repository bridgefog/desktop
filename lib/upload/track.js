...
class Track {
  ...
  addMusicToIPFS() {
    if (this.tags) {
      log('Adding:', this.tags.artist, this.tags.title)
    }
      ...
      return this.readTags().then(() => {
        return this.addImageToIPFS().then(() => {
          return this.addMusicToIPFS().then(() => {
            ...
            // log('metadata', this.musicPath, metadata)
            ...
            // log('metadata node', JSON.stringify(obj.asJSONforAPI()))
    ...
    return new Promise((resolve, reject) => {
      ...
      proc.on('close', code => {
        if (code === 0) {
          ...
          log('ERROR: [ipfs-add] exited ' + code)
          reject()
        }
